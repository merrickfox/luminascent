...
def _is_storefront_product_url(url: str) -> bool:
    ...
    if len(segments) != 1:
        return False
    slug = segments[0]
    if slug in ("", "index.html", "home"):
        return False
    return slug.endswith(".html") or "-" in slug
...
def collect_product_links(
    ...
    *,
    ...
    if len(scored) <= CARD_FALLBACK_THRESHOLD:
        fallback_hrefs = list(card_hrefs) or _all_hrefs(response)
        for href in fallback_hrefs:
            url = _normalize_url(base_url, href)
            if not url or not _same_domain(base_url, url) or _is_excluded(url):
                continue
            if url.rstrip("/") == base_url.rstrip("/"):
            ...
            if url in scored:
                continue
            if _is_storefront_product_url(url) or _product_path_score(url) > 0:
                scored[url] = max(scored.get(url, 0), 3)
