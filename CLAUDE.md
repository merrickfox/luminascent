...
**`scraper/QUIRKS.md` is the fix-history log** — every odd-looking guard/fallback in the scraper has an entry there with the site quirk that forced it and the invariant it protects. **Read it before deleting or "simplifying" any scraper guard, and add an entry after landing a new site-quirk fix.**
